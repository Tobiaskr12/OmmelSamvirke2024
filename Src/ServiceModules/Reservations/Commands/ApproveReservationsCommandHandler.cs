...
public class ApproveReservationsCommandHandler : IRequestHandler<ApproveReservationsCommand, Result<List<Reservation>>>
{
    ...
    public ApproveReservationsCommandHandler(
    ...
    public async Task<Result<List<Reservation>>> Handle(ApproveReservationsCommand request, CancellationToken cancellationToken)
    {
        ...
        // Validate ownership and approve each reservation
        ...
        if (updateResult.IsFailed) return Result.Fail<List<Reservation>>(ErrorMessages.GenericErrorWithRetryPrompt);

        // Create reservation approved email
        string reservationDetailsLink = $"https://www.ommelsamvirke.com/reservationer/oversigt?token={history.Token}";
        Result templateResult = _emailTemplateEngine.GenerateBodiesFromTemplate(
            Templates.Reservations.ReservationApprovedNotification,
