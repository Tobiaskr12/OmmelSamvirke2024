...
namespace Emails.Infrastructure.Tests;
...
public class AzureEmailServiceWrapperTests
{
    ...
    [Test]
    public async Task SendAsync_ValidEmail_ReturnsSuccessResult()
    {
        Email email = CreateValidEmail();

        // Mock EmailClient.SendAsync
        _emailClientMock = new Mock<EmailClient>(ValidConnectionString);
        var emailSendOperationMock = new Mock<EmailSendOperation>();
        emailSendOperationMock.SetupGet(op => op.HasValue).Returns(true);
        ...
        Result result = await serviceWrapper.SendAsync(email);
